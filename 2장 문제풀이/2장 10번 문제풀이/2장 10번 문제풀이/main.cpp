...
void Excersize_2_10(int solution, int answer)
{
	...
	if (solution == SHOW)
	{
		...
		printf("속도 변화량을 구해주면 %6.2lf = %6.2lf km/h /Δt. \n", change_of_velocity, frist_velocity);
		...
		printf("Δt을 구하는 공식Δt = Δv / a 에 값을 대입해주면  \n", final_change_of_velocity, acceleration, time);
		printf("(((%6.2lf m/s)-(%6.2lf m/s))*1000/3600) / (%6.2lf m/s^2)=(%6.2lf s) 라는 값이 나옵니다.", final_velocity, frist_velocity, acceleration, time);
